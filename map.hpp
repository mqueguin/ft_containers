...
namespace ft
{
	...
	class map
	{
		...
		public:
			...
			typedef ft::_const_tree_iterator<value_type> const_iterator;
		...
		public:
			...
			~map(void) {
				...
				for (; z != _tree.header && z->right != NULL; ) {
					for (; z->left != NULL; )
						z = z->left;
					if (z->right == NULL)
						break ;
				...
				for (; z != _tree.header; z = y) {
					y = z->parent;
					alloc.deallocate(z, 1);
					for (; y != _tree.header && y->right != NULL && y->right != z; ) {
						y = y->right;
						for (; y->left != NULL; )
							y = y->left;
					}
